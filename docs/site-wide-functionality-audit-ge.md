...
## Admin operations, media and fulfilment expansion — 2026-08-20

The Admin panel now provides a full manager workspace rather than link-only product-image editing. The product editor supports drag-and-drop, a standard multi-file chooser that works with phone gallery/camera pickers, managed image upload, an existing media library picker, up to eight images per product, main-image ordering and image removal before save. All file inputs are limited server-side to JPEG, PNG or WebP and 8 MB per file; uploads use the protected server route and managed storage instead of exposing storage credentials to the browser.

Category management now supports adding, editing and deleting categories. The delete guard rejects any category still referenced by a product, preventing accidental catalog orphaning. The order workspace adds live status queues, free-text search, expanded item/price context, recipient/address/date/time/notes, click-to-call, external route lookup and the existing protected status workflow; order deletion remains deliberately unavailable.

During live rollout, product data loading exposed that the pre-existing TypeScript gallery schema lacked its `productImages` production table. A non-destructive schema repair created the missing table and `(productId, sortOrder)` index. Metadata verification returned the table, then the published authenticated Admin loaded all 170 current products, categories and order count successfully. No product, order or customer data was changed by the repair.

## Checkout complete six-step UX reconstruction — 2026-08-20

The Checkout now follows the supplied reference’s information architecture more closely while retaining Flower’s Boutique styling. A three-part macro progress rail leads through six numbered cards: customer details, courier versus studio-pickup choice, a clickable delivery calendar and time window, recipient/address context, personalization, and final consent/confirmation. The live order summary remains sticky on desktop and keeps the cart thumbnail, quantity, subtotal, dynamic delivery price, total, next-step reassurance and return-to-cart action visible.

The courier/studio-pickup choice is functional rather than presentational. Courier continues to require an address and uses the existing threshold-based delivery fee. Studio pickup hides the address requirement, stores the fulfillment type as `pickup`, and sets the fee to zero server-side. Card messages and delivery instructions are combined into the existing persisted notes field; no new customer data is exposed or discarded. All newly visible content is translated in English, Georgian and Russian.

**Validation:** focused Checkout/fulfillment tests, TypeScript, **49 Vitest tests**, and `NODE_ENV=production pnpm build` pass. After the managed deployment retry, live Georgian Checkout verification rendered all six cards, the calendar, product summary and confirmation action. The non-destructive studio-pickup interaction changed the current order summary from **65 ₾** with delivery to **50 ₾** with **უფასო** delivery and replaced address fields with the studio address. Calendar next-month navigation was also confirmed in production; no order was submitted during QA.
