...
**Deployment follow-up:** the initial public request immediately after checkpoint `551d793a` still received the prior deployment’s 404 page for `/account`. The implementation has passed locally; the domain will be rechecked after rollout propagation before this follow-up is closed.
