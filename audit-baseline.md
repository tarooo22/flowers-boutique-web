...
## Authorized Account and Admin Surface Validation — 2026-08-12

An authorized, read-only browser validation traversed all four existing account tabs: profile information, addresses, orders, and favourites. The tab list retained its semantic tab controls, the neutral information-card surface, and visible edit/password/sign-out controls. The addresses, orders, and favourites tabs each rendered their intended empty-state card and did not expose a data-detail flow during this review. No profile field values, account data, customer addresses, order records, favourites, credentials, or mutations are reproduced in this report.

The authenticated administrator product overview loaded correctly with the Georgian control-room secondary label, the five existing section tabs, visible search/status/category filters, a localized category selector, the existing product count, and non-invoked create/edit/delete/availability controls. A cache-busting route load confirmed that the initially observed English header and selector labels were a stale browser bundle rather than an application or deployment failure. Desktop rendering keeps the structured table at `md` and above; the source has a separate one-column card layout below `md`, including wrapped, minimum-width action controls. No admin edit, delete, availability, feature, banner, category, setting, order, or payment action was invoked.

One confirmed source-level localization gap remains deliberately tracked for correction: the product category text inside both the desktop table and mobile product card currently reads `product.category?.nameEn` directly. The next narrowly scoped change will align these two render paths with the already verified Georgian-first category-label fallback, without altering records, filtering, layout, permissions, or route behavior.

## Final Localization Regression and Mobile Evidence Boundary — 2026-08-12

The tracked category-label gap is corrected in both product render paths: Georgian admin UI now resolves `product.category?.nameKa` first with `nameEn` fallback, while English preserves the inverse fallback. The focused UI contract now explicitly protects both occurrences. Full automated validation completed cleanly: **18 Vitest files passed, 3 opt-in integration files skipped; 97 tests passed, 10 skipped; TypeScript completed with no errors; and the production build completed successfully.** This change does not modify product records, catalog filters, permissions, order/payment behavior, BOG sandbox configuration, routes, or SEO content.

The 375px development-preview capture correctly rendered the existing Georgian access-denied state for an unauthenticated context, including its bounded, readable mobile layout. The authenticated production session remains isolated in the user browser, while the preview capture session is deliberately separate; therefore this capture cannot truthfully stand in for an authenticated mobile-admin inspection. The responsive implementation is source-confirmed through its `md` desktop table/mobile-card branch, but the final checklist keeps a distinct pending item for a true authenticated 375px read-only validation rather than overstating the evidence.

The `ui.header-contract` suite now additionally protects the responsive split itself: the desktop product table remains hidden below `md`, the mobile single-column card list remains visible below `md`, and availability/edit/delete controls retain their wrapping layout with explicit 120px, 100px, and 80px minimum widths. After this added contract, the complete regression run reports **18 Vitest files passed, 3 opt-in integration files skipped; 98 tests passed, 10 skipped; TypeScript has no errors; and the production build succeeds.**

An attempted same-origin, `noindex` 375×812 QA wrapper loaded as a static production page, but its embedded `/admin` route returned the storefront 404 state in the production iframe context. This does not represent a valid authenticated mobile-admin result and is not used as evidence. The wrapper is being removed; the remaining authenticated 375px confirmation remains explicitly pending.

On the connected, authenticated production route `https://flower-shop-jx9auvvz.manus.space/admin?mobile-evidence=1`, the Georgian header secondary label rendered as **„მართვის ცენტრი“**, and the visible category filter options rendered as **„საქორწინო თაიგული“**, **„თაიგული“**, and **„ცალკეული ყვავილები“**. The browser automation’s DevTools/device-toolbar keyboard shortcuts did not change the inspected viewport, so this desktop capture is not treated as 375px evidence. No administrative mutation was performed and no private order or customer detail was recorded.


## Master prompt delivery/order-request constraints (2026-08-12)

The supplied `pasted_content_3.txt` requires one honest flow: either a real transactional checkout or a clearly labelled Order Request; a non-functional request page must not be presented as completed online checkout. It identifies current inconsistencies that require one verified source of truth: checkout shows GEL 10 delivery, Delivery page states GEL 5 and free delivery above GEL 150, and a GEL 250 cart still receives GEL 10; opening hours and one delivery-page phone number also conflict with the main number +995 511 55 56 50. Fees must be calculated server-side. The prompt also requires semantic main landmarks on About, Delivery, Returns, and Checkout, reliable server-side contact submission instead of mailto, 44x44px mobile controls, and safe space so the Manus badge cannot overlap mobile navigation or checkout CTAs. Source: `/home/ubuntu/upload/pasted_content_3.txt`, lines 81-129.

Current implementation evidence: `Checkout.tsx` displays and locally computes a hardcoded GEL 10 delivery fee; `Delivery.tsx` states GEL 5 and free delivery above GEL 150; `orders.create` canonicalizes item prices but does not accept or persist fulfillment type/delivery fee; `orders.checkout` accepts fulfillment type but also recomputes only item subtotal. The next safe implementation should centralize the verified delivery policy, pass fulfillment type through the order-request mutation, calculate the authoritative fee on the server, and preserve BOG sandbox mode.
