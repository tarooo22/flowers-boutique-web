...
export function CheckoutView() {
  ...
  useEffect(() => { fetch("/api/storefront/delivery-policy").then((response) => response.ok ? response.json() : null).then((payload) => setDeliveryPolicy(payload?.policy ?? null)).catch(() => setDeliveryPolicy(null)); }, []);
  const deliveryCopy = deliveryPolicy ? (lang === "ka" ? deliveryPolicy.noticeKa : lang === "ru" ? deliveryPolicy.noticeRu : deliveryPolicy.noticeEn) : t("checkout.deliveryMethodCopy");
