...
function ProductManagementCard({ product, patchProduct, onEdit, onDelete, busy }: { product: AdminProduct; patchProduct: (id: string, patch: Record<string, unknown>) => void; onEdit: (product: AdminProduct) => void; onDelete: (id: string) => void; busy: string | null }) {
  const isSaving = busy === `product:${product.id}`;
  return <article className="min-w-0 rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[var(--shadow-card)]"><div className="flex min-w-0 items-start gap-3"><MediaThumb media={product.images[0]} fallback={product.image} /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><p className="break-words text-[14px] font-semibold leading-snug">{product.name}</p><p className="mt-0.5 break-words text-[12px] text-[var(--muted)]">{product.nameEn}</p></div><span className="shrink-0 rounded-full bg-[var(--surface-sand)] px-2 py-1 text-[10px] font-bold text-[var(--muted)]">{product.images.length || 0} ფოტო</span></div><p className="mt-2 break-words text-[11px] text-[var(--muted)]">{product.category}</p></div></div><div className="mt-4 grid gap-3 border-t border-[var(--line)] pt-4"><label className="grid gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">ფასი<input aria-label={`Price for ${product.name}`} type="number" min={0} defaultValue={product.price} disabled={isSaving} onBlur={(event) => { const value = Number(event.target.value); if (value !== product.price) void patchProduct(product.id, { price: value }); }} className="h-11 w-full rounded-[var(--radius)] border border-[var(--line)] bg-white px-3 text-[14px] font-semibold tabular-nums text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--action)] disabled:opacity-50" /></label><div className="flex min-h-11 items-center justify-between gap-3 rounded-[var(--radius)] bg-[var(--surface-sand)] px-3"><span className="min-w-0 text-[12px] font-semibold text-[var(--ink)]">{product.available ? "მარაგშია" : "არაა მარაგში"}</span><Toggle checked={product.available} disabled={isSaving} onChange={(value) => patchProduct(product.id, { available: value })} /></div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => onEdit(product)} className={`${minorButton} min-h-11`}>რედაქტირება</button><button type="button" disabled={busy === `delete:${product.id}`} onClick={() => onDelete(product.id)} className={`${minorButton} min-h-11 border-[var(--action)]/35 text-[var(--action-deep)] hover:bg-[var(--action)]/10`}>წაშლა</button></div></div></article>;
}
...
function Toolbar({ query, setQuery, placeholder }: { query: string; setQuery: (value: string) => void; placeholder: string }) { return <div className="flex min-w-0 w-full items-center gap-2 rounded-[var(--radius)] border border-[var(--line)] bg-white px-3"><SearchIcon className="h-[18px] w-[18px] shrink-0 text-[var(--muted)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={placeholder} className="h-11 min-w-0 w-full bg-transparent text-[13.5px] outline-none placeholder:text-[var(--muted-2)]" /></div>; }
...
const selectClass = "h-11 min-w-0 w-full rounded-[var(--radius)] border border-[var(--line)] bg-[var(--surface)] px-3 text-sm outline-none focus:ring-2 focus:ring-[var(--action)]";
