...
export default function Home() {
  ...
  const thirdRail = featured.slice(8, 16).length ? featured.slice(8, 16) : featured.slice(0, 8);
  ...
  const productRail = (items: any[], label: string, title: string, priority = false) => (
    <section className="am-home-rail" aria-labelledby={label}>
      ...
      {productsQuery.isLoading ? (
        <div className="am-product-rail am-product-rail--skeleton" aria-busy="true">
          {Array.from({ length: 4 }).map((_, index) => <span key={index} />)}
        </div>
      ) : items.length ? (
        <div className="am-product-rail">
    ...
    <div className="am-site">
      ...
      <main id="main-content" className="am-home">
        <section className="am-home-hero" aria-labelledby="am-home-hero-title">
          ...
          <div className="am-home-hero__content">
            ...
            <dl className="am-hero-metrics">
              <div><dt>{ka ? "მიწოდება თბილისში" : "Tbilisi delivery"}</dt><dd>₾{DELIVERY_FEE_GEL}</dd></div>
              <div><dt>{ka ? "უფასო მიწოდება" : "Free delivery"}</dt><dd>₾{FREE_DELIVERY_THRESHOLD_GEL}+</dd></div>
              <div><dt>{ka ? "თაიგულები კატალოგში" : "Bouquets in catalog"}</dt><dd>{products.length || "—"}</dd></div>
            </dl>
          </div>
        </section>
        ...
        <section className="am-occasion" aria-labelledby="am-occasion-title">
          <div className="am-shell">
            ...
            <div className="am-occasion__chips">
              ...
              {categories.map((category: any) => <Link key={category.id} href={`/catalog?category=${category.id}`}>{ka ? category.nameKa : category.nameEn}</Link>)}
            </div>
          </div>
        </section>
        ...
        <div className="am-shell am-home__body">
          ...
          {productRail(thirdRail, "am-selection-title", ka ? "არჩეული თაიგულები" : "Selected bouquets")}

          <section className="am-builder-promo" aria-labelledby="am-builder-title">
            <div>
            ...
            <img src="/manus-storage/flowers-boutique-experience-event-styling_07114441.jpg" alt="" />
          </section>
