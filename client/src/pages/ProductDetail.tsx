...
export default function ProductDetail() {
  ...
  return (
    <div className="p2-product-page p3-product-amelie-page zip-product-page min-h-screen bg-[#f7f2e9] text-[#181614]">
      <Navbar />
      <main>
        <div className="fb-page-shell fb-breadcrumbs fb-product-breadcrumbs p3-product-amelie-breadcrumbs"><Link href="/">{ka ? "მთავარი" : "Home"}</Link><ChevronRight size={14} /><Link href="/catalog">{ka ? "კატალოგი" : "Catalog"}</Link><ChevronRight size={14} /><span>{name}</span></div>
        <section className="fb-page-shell fb-product-layout p3-product-amelie-layout zip-product-layout" aria-labelledby="product-title">
          <div className="fb-product-gallery p3-product-amelie-gallery" aria-label={ka ? "პროდუქტის ფოტოები" : "Product images"} data-gallery-count={gallery.length}>
          ...
          <div className="fb-product-copy p3-product-amelie-copy">
            <p className="fb-eyebrow">FLOWER'S BOUTIQUE · SIGNATURE</p>
            <h1 id="product-title" className="fb-display">{name}</h1>
            ...
            <div className="p3-product-amelie-purchase-card zip-product-purchase-card" data-availability={product.isAvailable ? "available" : "unavailable"}>
              <div id="product-price" className="fb-product-price-row" aria-live="polite"><strong>{price}</strong>{quantity > 1 && !product.priceOnRequest && <span>{ka ? `ჯამი: ₾${((Number(displayMin) || 0) * quantity).toFixed(0)}` : `Total: ₾${((Number(displayMin) || 0) * quantity).toFixed(0)}`}</span>}</div>
              <div id="product-availability" className={`fb-product-availability ${product.isAvailable ? "is-available" : "is-unavailable"}`} aria-live="polite">{product.isAvailable ? <><Check size={16} /> {ka ? "მარაგშია" : "In stock"}</> : <><AlertCircle size={16} /> {ka ? "ამოიწურა" : "Out of stock"}</>}</div>
              ...
              {selectedVariant && <p className="p3-product-amelie-selection" aria-live="polite">{ka ? `არჩეული ფერი: ${selectedVariant.colorNameKa || selectedVariant.colorNameEn}` : `Selected colour: ${selectedVariant.colorNameEn || selectedVariant.colorNameKa}`}</p>}
              ...
              <button type="button" className="fb-product-add p3-product-amelie-primary-cta" disabled={!product.isAvailable || selectedVariant?.available === false} onClick={addProduct} aria-describedby="product-price product-availability">{added ? <><Check size={19} /> {ka ? "დაემატა" : "Added"}</> : <><ShoppingBag size={19} /> {ka ? "კალათაში დამატება" : "Add to cart"}</>}</button>
              <div className="p3-product-amelie-delivery-note" aria-label={ka ? "მიწოდების პირობები" : "Delivery policy"}><Truck size={18} aria-hidden="true" /><p>{ka ? `თბილისში მიწოდება ₾${DELIVERY_FEE_GEL} · უფასო ₾${FREE_DELIVERY_THRESHOLD_GEL}-დან · თვითგატანა უფასოა` : `Tbilisi delivery ₾${DELIVERY_FEE_GEL} · Free from ₾${FREE_DELIVERY_THRESHOLD_GEL} · Pickup is free`}</p></div>
        ...
        {related.length > 0 && <section className="fb-page-shell fb-related p3-product-amelie-related zip-product-related" aria-labelledby="related-products-title"><div className="fb-section-heading"><div><p className="fb-eyebrow">YOU MAY ALSO LIKE</p><h2 id="related-products-title" className="fb-display">{ka ? "სხვა რჩეული თაიგულები" : "More from the collection"}</h2></div><Link href="/catalog">{ka ? "ყველას ნახვა" : "View all"}<ChevronRight size={15} /></Link></div><div className="fb-related-grid">{related.map((item: any) => { const relatedName = getProductName(item, language); return <Link key={item.id} href={`/product/${item.id}`} className="fb-related-card p3-product-amelie-related-card"><FlowerImage src={item.imageUrl} alt={relatedName} className="h-full w-full object-cover" /><div><h3>{relatedName}</h3><span>{priceText(item.priceMin, item.priceMax, Boolean(item.priceOnRequest), ka)}</span></div></Link>; })}</div></section>}
      </main>
      <div className="fb-product-sticky p3-product-amelie-sticky" aria-label={ka ? "კალათაში დამატების სწრაფი მოქმედება" : "Quick add to cart"}><span>{price}</span><button type="button" onClick={addProduct} disabled={!product.isAvailable || selectedVariant?.available === false}><ShoppingBag size={17} /> {ka ? "დამატება" : "Add"}</button></div>
      <Footer /><CartDrawer />
    </div>
  );
}
